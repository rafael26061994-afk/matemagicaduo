...
function getTabuadaRangeByLevel(level) {
    switch (level) {
        case 'easy':
            return { min: 0, max: 5, label: 'Fácil (0–5)' };
        case 'medium':
            return { min: 6, max: 10, label: 'Médio (6–10)' };
        case 'advanced':
            return { min: 11, max: 20, label: 'Difícil (11–20)' };
        default:
            return { min: 0, max: 20, label: 'Completo (0–20)' };
    }
}
...
`;
    ...
    const renderRangeTexts = () => {
        ...
        if (rangeLine) rangeLine.textContent = `Nível: ${r.label} — Tabuadas ${r.min} a ${r.max}`;
        if (footerTip) footerTip.textContent = `Dica: a trilha percorre as tabuadas de ${r.min} a ${r.max} em uma ordem aleatória.`;
