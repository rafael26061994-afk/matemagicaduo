...
function loadMultiplicationConfig() {
    try {
        ...
        if (Array.isArray(cfg.trailOrder) && cfg.trailOrder.length === 21) gameState.multiplication.trailOrder = cfg.trailOrder;
...
function ensureTrailOrder() {
    if (!Array.isArray(gameState.multiplication.trailOrder) || gameState.multiplication.trailOrder.length !== 21) {
        gameState.multiplication.trailOrder = shuffleArray(rangeInclusive(0, 20));
        gameState.multiplication.trailIndex = 0;
    }
    // garante índice válido
    if (gameState.multiplication.trailIndex < 0 || gameState.multiplication.trailIndex > 20) {
        gameState.multiplication.trailIndex = 0;
    }
}
...
function advanceTrailTabuada() {
    ...
    if (gameState.multiplication.trailIndex > 20) {
        // terminou o ciclo 0–20 (ordem aleatória) → cria uma nova ordem
        gameState.multiplication.trailOrder = shuffleArray(rangeInclusive(0, 20));
        gameState.multiplication.trailIndex = 0;
    }
...
function prepareRoundMultipliers0a20() {
    gameState.multiplication.roundMultipliers = shuffleArray(rangeInclusive(0, 20));
    gameState.multiplication.roundPos = 0;
}
function getNextRoundMultiplier() {
    if (!Array.isArray(gameState.multiplication.roundMultipliers) || gameState.multiplication.roundMultipliers.length !== 21) {
        prepareRoundMultipliers0a20();
    }
    ...
    overlay.innerHTML = `
        <div class="mm-modal" role="dialog" aria-modal="true" aria-label="Configuração da multiplicação">
            <div class="mm-modal-header">
                <h2>Multiplicação — Tabuada 0 a 20</h2>
                <button class="mm-close" type="button" aria-label="Fechar">✕</button>
            </div>
            <p class="mm-sub">Escolha como você quer treinar:</p>

            <div class="mm-actions">
                <button type="button" class="mm-btn mm-primary" data-mm="trail">🗺️ Trilha automática (0–20)</button>
            ...
            <div class="mm-direct hidden" aria-label="Escolher tabuada">
                <p class="mm-sub2">Selecione a tabuada:</p>
                <div class="mm-grid" id="mm-grid"></div>
            </div>

            <div class="mm-footer">
                <small>Dica: a trilha percorre todas as tabuadas de 0 a 20 em uma ordem aleatória.</small>
            </div>
        </div>
    `;
...
function openMultiplicationConfig(level) {
    ...
    // Atualiza sugestão do estado atual no modal (se for trilha)
...
function startGame(operation, level) {
    ...
    // --- Configuração especial: Tabuada da Multiplicação (0–20) ---
    if (operation === 'multiplication' && gameState.multiplication && (gameState.multiplication.mode === 'direct' || gameState.multiplication.mode === 'trail')) {
        ...
        // Para tabuada, sempre fazemos a rodada completa 0–20 (21 itens)
        gameState.totalQuestions = 21;
        prepareRoundMultipliers0a20();
    }
...
function handleAnswer(selectedAnswer, selectedButton) {
    ...
    if (isCorrect) {
        ...
        // Se errou, o tempo fica mais lento (ajuda o aluno a se recuperar)
        gameState.timeStep = gameState.slowTimeStep;
...
function attachEventListeners() {
    ...
    document.addEventListener('keydown', (e) => {
        ...
        if ((e.key === 'r' || e.key === 'R') && gameState.isVoiceReadActive && gameState.currentQuestion) {
            e.preventDefault();
            speak(`Questão ${gameState.questionNumber}. ${gameState.currentQuestion.question}`);
        }
    });
